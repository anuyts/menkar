...
class (Functor t, Foldable t, Traversable t, CanSwallow (Term sys) t) =>
  AllowsMetas sys descr t | t -> sys, t -> descr where
...
forThisClassWithMetas1 :: forall sys f s v .
  (forall descr t w .
  ...
  s v
forThisClassWithMetas1 f (ForSomeClassWithMetas x) = f x
